...
# le montage — 1 % sur toutes les distances (voir optics.py). Ce script est
...
if optics.source(MONTAGE) != MONTAGE:
    print(f"ERREUR : le montage '{MONTAGE}' n'a jamais ete calibre — il n'y a "
          "rien a verifier.")
    print(f"  python calibrate.py --montage {MONTAGE}")
    raise SystemExit
...
if optics.ORIENTATION == "radiale" and MONTAGE != "nue_air":
    print(f"Rappel : {optics.sensibilite_glissement():.1f} % d'erreur de distance "
...
while True:
    ...
    if touche == ord("v"):
        ...
        if erreur < 0.5:
            ...
            print(">>> INCOMPATIBLE : refais calibrate.py sur ce montage.")
        ...
        if erreur >= 0.5 and optics.ORIENTATION == "radiale" and MONTAGE != "nue_air":
            if rms_y > 2 * rms_x:
