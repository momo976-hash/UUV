...
# copied there.
#
#     python cylindrical_wall_model.py     shows the table
#
# WHAT IT ESTABLISHES
...
# below these
# previsions. That gap remains open.
#
...
R1, R2 = 0.02475, 0.02900          # inner / outer radii of the tube
N_AIR, N_AC, N_WATER = 1.0, 1.49, 1.33
PUPIL = 0.00315                    # measured off-axis offset, towards the wall
FX_BARE, FY_BARE = 615.56, 615.09  # bare camera, measured today
CX, CY = 320.0, 240.0

def _cylinder_intersection(o, d, R):
    """Intersection radius / cylinder of axis X, walking towards +z."""
    a = d[1]**2 + d[2]**2
    b = 2*(o[1]*d[1] + o[2]*d[2])
    c = o[1]**2 + o[2]**2 - R*R
    disc = b*b - 4*a*c
...
def project(W):
    ...
    for _ in range(60):
        ...
        return None
    return np.array([FX_BARE*dx + CX, FY_BARE*dy + CY])
...
if __name__ == "__main__":
    ...
    for Z in (0.3, 0.4, 0.5, 0.6, 0.75, 1.0, 1.5, 2.0, 3.0, 10.0, 1000.0):
        ...
        g = (p[0] - CX) / (FX_BARE * eps)
        print(f"{Z:13.2f} {g:15.4f} {FX_BARE * g:13.1f}")
    print("-" * 44)
    # Check: the axial direction must give the plane-parallel slab back. It
    # is this test that caught the sign error on the normal.
    n, d0 = N_WATER, R1 - PUPIL
    worst_gaps = []
    for Z in (0.3, 0.75, 2.0, 1000.0):
        p = project(np.array([eps * Z, 0.0, Z]))
        trace = (p[0] - CX) / (FX_BARE * eps)
        analytic = n * Z / (Z + (d0 + (R2 - R1) * (1 - n / N_AC)) * (n - 1))
        worst_gaps.append(abs(trace / analytic - 1))
    print(f"  Plane-slab check: max gap {100*max(worst_gaps):.2f} % "
