...
# Calibrations enregistrees par calibrate.py --montage <nom>
...
def _lire_montage_local():
    ...
    for ligne in texte.splitlines():
        ...
        if ligne in MONTAGES:
            return ligne
        print(f"[optics] {FICHIER_MONTAGE_LOCAL.name} : '{ligne}' n'est pas "
...
def _resoudre_montage():
    ...
    if not os.environ.get("UUV_MONTAGE_MUET"):
        print(f"[optics] montage non regle sur cette machine, on prend "
              f"'{defaut}'.")
        print("[optics]   si la camera est dans le tube ou dans l'eau, les "
...
def charger(montage=None, silencieux=False):
    ...
    if not silencieux and montage != "nue_air":
        ...
        print(f"[optics] montage '{montage}' pas encore calibre, "
...
def verifier_montage():
    ...
    soucis.append(
        ...
        "calibrate.py --montage tube_air en deduit le decentrement reel.")
    ...
    for montage in MONTAGES:
        ...
        if fichier.exists():
            ...
            lignes.append(f"  {montage:10} {'—':>8}     pas encore mesure  "
                          f"(calibrate.py --montage {montage})")
