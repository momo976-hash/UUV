...
# --- 4) Draw the result ---
if ids is not None:
    print(f"OK: {len(ids)} tag(s) detected, id = {ids.flatten().tolist()}")
    ...
    for c, tag_id in zip(corners, ids.flatten()):
        ...
        if ok:
            ...
            print(f"  tag {tag_id}: estimated distance = {distance:.2f} m")
...
print(f"Result image saved here: {path}")
