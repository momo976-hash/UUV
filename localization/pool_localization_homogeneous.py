...
# Meme resultat que localisation_piscine.py, mais ecrit avec des
...
#   T_A_B = pose du frame B vu depuis le frame A (convertit un point B -> A).
#   solvePnP  -> T_camera_tag   (le tag vu depuis la camera)
#   la carte  -> T_piscine_tag  (le tag vu depuis la piscine, connu)
...
while True:
    ...
    if ids is not None:
        ...
        for c, tag_id in zip(coins, ids.flatten()):
            ...
            T_camera_tag = transformation(R, tvec)              # tag vu depuis la camera
